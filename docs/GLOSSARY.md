...
free-text opinion. See `docs/ARCHITECTURE.md` §5 and
`reference/schema/audit.schema.json`.
...
A candidate is never treated as a verdict. See `docs/ARCHITECTURE.md`
§3.
...
`verdict == "accepted"`. See `docs/ARCHITECTURE.md` §4 and
`reference/schema/receipt.schema.json`'s `certificate` field.
...
presented as contract-grade has to carry evidence (a control) that its
oracle can fail. See `docs/PROVENANCE.md` §1 and
`reference/autoprover_ref/audit.py`.
...
`reference/schema/receipt.schema-2.0.0.json`'s `claim_id` field.
...
`reference/schema/receipt.schema-2.0.0.json`'s `control` field and
...
`reference/schema/receipt.schema-2.0.0.json`'s per-obligation `coverage`
field.
...
true on every explored state; `vacuous` means the precondition never
fired, and exploration was exhaustive; `not-exercised` means the
precondition never fired, but exploration was bound-truncated so that
outcome can't be attributed to the system's semantics; `failed` means
the property broke. `vacuous` and `not-exercised` are first-class and
distinct from `held`, so an obligation that was never really tested can
never look identical to one that was. See
`reference/schema/receipt.schema.json`'s `obligations[].status` field
and `reference/README.md`'s "The model checker" section.
...
its design lineage to be trusted directly." See `docs/ARCHITECTURE.md`
§4 and `reference/autoprover_ref/kernel_gate.py`.
...
label. See `docs/ARCHITECTURE.md` §5 and
...
`reference/schema/receipt.schema.json`'s `obligations` field and
`docs/INTERFACES.md` property 2.
...
it match what we said it proved" gap. See `docs/ARCHITECTURE.md` §1.
...
`docs/ARCHITECTURE.md` §6 and [ADR-003](decisions/ADR-003-monotone-ratchet.md).
...
candidate, which checker version, what verdict, and (on acceptance) a
re-verifiable certificate. Never a log message meant for a human to
interpret. See `docs/ARCHITECTURE.md` §7.

## scoped vs. full

The honesty flag `corpus/CATALOG.md` attaches to every module. `full`
means the module proves the general result its name suggests. `scoped`
means the module is deliberately restricted — a fixed finite instance,
one direction of a biconditional, a safety-only property with no
liveness claim, an assumed rather than proved hypothesis — with a few
words on what is restricted, taken from the module's own header. See
`corpus/CATALOG.md`'s introduction.
...
`reference/schema/receipt.schema-2.0.0.json`'s `subject` and
`toolchain` fields.
...
record — never that the statement is true. See `docs/ARCHITECTURE.md`
§1.
...
though the kernel is green. See `docs/ARCHITECTURE.md` §5.

## watched fail

**Informal phrase, not a schema status.** Shorthand used in prose (the
honesty notes, `docs/PROVENANCE.md`) for a control receipt whose
prediction was `red` and whose observation was also `red` — a check
that somebody actually watched fail, as opposed to one that has only
ever been observed passing. It does not appear as an enum value
anywhere in the schemas; the machine-readable form of the same idea is
the `control` block's `expectation`/`observed` pair. See
`docs/PROVENANCE.md` §2 and [ADR-004](decisions/ADR-004-abstain-vs-fail.md).
