...
| ByzantineTightness | At n = 3f, two quorums certify contradictory outcomes — a static instance exhibiting two uniform but contradictory certified views; this makes 3f+1 necessary only given the unformalized liveness premise that usable quorums must have size ≤ n − f. | Classical (Pease, Shostak and Lamport, 1980). | scoped — the tightness half is the single f = 1, n = 3 witness (nodes `[1, 2, 3]`, two size-2 quorums, the shared node faulty); no disagreeing instance is constructed for general f; the module separately proves 2f+1-sized quorums stay safe at n = 3f |
...
| TwoGeneralsBoundedImpossibility | No content-blind (ack-only) k-round protocol over a lossy channel can guarantee agreement, attack-on-full-delivery, and retreat-on-silence together. | Classical (Akkoyunlu, Ekanadham and Huber, 1969; Gray, 1978). | scoped — general in k, but over the canonical alternating schedule and restricted to protocols whose decisions depend only on the per-round delivery outcomes, not on message content (a strictly smaller class than all deterministic protocols); the WLOG reduction from an arbitrary message schedule is not formalized |
...
| TestingPreorder | The preorder induced by an arbitrary may-testing pass relation is reflexive and transitive (test-set containment); DNH-attributed, machinery not formalized. | Classical (De Nicola and Hennessy, 1984). | scoped — pass relation kept abstract; no LTS/observer/computation machinery |
...
| MerkleInclusion | Merkle inclusion-proof soundness under an injective hash plus leaf/node domain separation. | Classical (Merkle, 1979). | scoped — hash injectivity and domain separation are assumed hypotheses, not proved; no cryptographic collision-resistance claim is made |
...
7 areas, 74 modules (Concurrency 8, Distributed 18, Order 8, Probability 13, Processes 8, Reliability 8, Security 11); 40 flagged `full`, 34 flagged `scoped`.
